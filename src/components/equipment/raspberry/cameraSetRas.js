...
const rencolor = ["#ff0" ,"#5063ee" ,"#ED2F2F" ,"#4ec9b0"]  //0为初始化颜色，1为防区1颜色，2为防区2颜色
...
let lastxy = []  //移动时上次的
let xinzb = [] //更新时点的坐标
 class CameraSetRas extends Component {
    constructor(props) {
      super(props);
      this.state = {
    ...
    componentWillReceiveProps(nextProps, nextContext) {
        if(nextProps.camerdat.equipData){
            if(this.props.camerdat.equipData!= nextProps.camerdat.equipData){
                this.handleTabChange(nextProps.camerdat.equipData)
                this.oneTypeSelectFun(nextProps.camerdat.equipData);
                this.setState({
                    devid:nextProps.camerdat.equipData.eid,
                    basemap:nextProps.camerdat.equipData.basemap,
                },()=>{
                    this.hanleRas();
                })
            }
    ...
    hanleRas=()=>{
      axios.ajax({
          method:"get",
          url:window.g.loginURL+"/api/rasp/webbackground",
          data:{
          ...
          if(res.success){
              if(res.data){
                  setTimeout(()=>this.hanleResulet(res.data),3000);
              }
          }
      });
    };
    ...
    render(){
      ...
      const defopt = [
        ...
        <div className="listItemWrap">
            <Button type="dashed" className="again" onClick={this.hanleRas}>
              重新获取底图
            </Button>
        </div>
      ];
      return(
        <Row className="filed">
            <canvas
              ...
              id="cavcontainer"
              style={{
                backgroundImage: "url(" + `${window.g.imgUrl+this.state.basemap+"?t="+this.state.nowTime}` + ")",
